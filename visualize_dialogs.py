...
def create_dialog_only_image(image_path: str, merged_json_path: str, 
                            output_path: str = None) -> str:
    """
    创建只显示对话框的图像
    ...
    if output_path is None:
        base_name = os.path.splitext(os.path.basename(image_path))[0]
        output_dir = os.path.dirname(merged_json_path)
        output_path = os.path.join(output_dir, f"dialogs_only_{base_name}.jpg")
...
def main():
    ...
    parser.add_argument('--comparison', '-c', action='store_true', 
                       help='创建对比图像')
    parser.add_argument('--dialogs-only', '-d', action='store_true',
                       help='只显示对话框边界')
    ...
    try:
        if args.comparison:
            ...
            output_path = create_dialog_only_image(
                args.image_path,
                args.merged_json,
                args.output
            )
            print(f"对话框专用图像已保存至: {output_path}")
        else:
